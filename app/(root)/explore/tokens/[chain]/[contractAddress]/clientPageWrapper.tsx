...
const Page = async ({ contractAddress, data, chain }: any) => {
    ...
    const updateChartData = async (range: string) => {
        ...
        const selectedTimeRange = TIME_RANGES.find(r => r.label === range)?.value || 86400;
        const endTime = Math.floor(Date.now() / 1000);
        const startTime = endTime - selectedTimeRange;

        try {
            const newData = await fetch(`/api/charts?chainId=${chain?.chainId}&contractAddress=${contractAddress}&startTime=${startTime}&endTime=${endTime}`)
